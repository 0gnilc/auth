...
CREATE TABLE IF NOT EXISTS az_role (
    ...
    code varchar(320) NOT NULL COMMENT '角色标识，业务长度上限 255，预留逻辑删除后缀空间',
...
ALTER TABLE az_role
    MODIFY COLUMN code varchar(320) NOT NULL COMMENT '角色标识，业务长度上限 255，预留逻辑删除后缀空间';
...
CREATE TABLE IF NOT EXISTS az_permission (
    ...
    create_time datetime(6) NOT NULL COMMENT '创建时间（UTC）',
    update_time datetime(6) DEFAULT NULL COMMENT '修改时间（UTC）',
    code varchar(320) NOT NULL COMMENT '权限标识，业务长度上限 255，预留逻辑删除后缀空间',
...
ALTER TABLE az_permission
    MODIFY COLUMN code varchar(320) NOT NULL COMMENT '权限标识，业务长度上限 255，预留逻辑删除后缀空间';
...
CREATE TABLE IF NOT EXISTS az_menu (
    ...
    access_code varchar(320) DEFAULT NULL COMMENT '后端权限标识，业务长度上限 255，预留逻辑删除后缀空间',
    name varchar(320) NOT NULL COMMENT '菜单名称，业务长度上限 255，预留逻辑删除后缀空间',
    path varchar(560) DEFAULT NULL COMMENT '路由路径，业务长度上限 500，预留逻辑删除后缀空间',
...
ALTER TABLE az_menu
    MODIFY COLUMN access_code varchar(320) DEFAULT NULL COMMENT '后端权限标识，业务长度上限 255，预留逻辑删除后缀空间',
    MODIFY COLUMN name varchar(320) NOT NULL COMMENT '菜单名称，业务长度上限 255，预留逻辑删除后缀空间',
    MODIFY COLUMN path varchar(560) DEFAULT NULL COMMENT '路由路径，业务长度上限 500，预留逻辑删除后缀空间';
