...
客户端应用是与后端交互的独立前端应用，例如当前的 `admin` 管理端；它用于区分客户端配置范围，不等同于登录身份、访问主体或权限凭证。

### 后台管理员用户（Admin User）

后台管理员用户是后台管理系统中可登录的人员身份，拥有可维护的账号资料，例如昵称等展示信息。每个后台管理员用户对应一个 RBAC 授权主体，必须拥有后台访问基线角色，并通过该授权主体参与其他角色、权限和菜单授权。

### 当前后台管理员（Current Admin User）

当前后台管理员是当前后台管理员会话所代表的后台管理员用户，是读取本人资料、维护本人基本信息、变更本人密码以及解析本人角色和访问权限时的访问主体。

### 当前后台管理员导航路由树（Current Admin Navigation Route Tree）

当前后台管理员导航路由树是当前后台管理员获准使用的后台导航结构，只包含从根节点可达的有效菜单；缺少有效父级的非根菜单不属于该路由树。已获授权的空目录仍属于该路由树；它不同于供菜单管理使用的全量菜单树。

### 菜单层级（Menu Hierarchy）

菜单层级是由根菜单和各自具有唯一父级的非根菜单组成的无环结构；菜单不能以自身或自己的后代作为父级。

### 菜单授权层级闭包（Menu Authorization Hierarchy Closure）

菜单授权层级闭包是角色菜单授权集合只由存在且未删除的菜单组成，并同时包含每个获授菜单到根节点所需父级的约束，使任何获授菜单都具有完整导航层级。禁用菜单仍可保留在授权集合中。

### 菜单禁用（Menu Disablement）

菜单禁用是暂时停止菜单及其后代参与导航的状态，不终止已存在的角色菜单授权；重新启用后，原有授权可以恢复生效。

### 菜单子树删除（Menu Subtree Removal）

菜单子树删除是删除一个菜单节点及其全部后代节点，并终止角色对这些菜单授权的整体操作，不保留脱离父级的后代菜单。

### 当前后台管理员自助维护（Current Admin Self-Service）

当前后台管理员自助维护是当前后台管理员维护本人基本信息和登录密码的能力。它要求存在有效的后台管理员会话，以该会话确定目标用户，不接受由调用方指定其他后台管理员用户。自助维护所需权限通过后台访问基线角色授予。

### 后台访问基线角色（Admin Access Baseline Role）

后台访问基线角色是每个后台管理员用户必须拥有且不能移除的内置 `admin` 角色。它承载后台管理员读取本人资料、角色、按钮访问码和基础导航，以及维护本人基本信息和登录密码所需的权限；其他角色在此基础上叠加更具体的业务权限。

### 国际化配置管理员（I18n Manager）

国际化配置管理员是能够查询和维护客户端动态国际化配置的内置后台管理角色，对应角色代码 `i18n-manager`；该角色身份不可修改或删除，普通后台管理员不会因拥有后台访问基线角色而自动获得该能力。

### 后台管理员管理（Admin User Administration）

后台管理员管理是由有权操作的后台管理员选择目标后台管理员用户并维护其账号资料和角色绑定的管理能力。它与当前后台管理员自助维护相区分。
