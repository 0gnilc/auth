...
class PermissionCacheTransactionIT {
    ...
    @Autowired
    private PermissionCacheRedisResetTransport redisResetTransport;
    ...
    void cleanCommittedRelations() {
        new TestEnvironmentGuard().verifyDatabase(environment, dataSource);
    ...
    @Test
    void replacesRelationsAndResetsAffectedUsersOnlyAfterCommit() {
        long roleId = 7101L;
        long userId = 7201L;
        seedUserRole(userId, roleId);
        seedRolePermission(roleId, 7301L);
        RolePermissionDto replacement = replacement(roleId, 7302L, 7303L, 7303L);

        transactions.executeWithoutResult(status -> {
            rolePermissionService.updateRolePermission(replacement);
    ...
    @Test
    void rollbackPreservesRelationsAndSuppressesLocalAndRedisResets() {
        long roleId = 7102L;
        long userId = 7202L;
        seedUserRole(userId, roleId);
